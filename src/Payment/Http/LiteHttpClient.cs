...
namespace Payment.Http
{
    public class LiteHttpClient
    {
        ...
        public LiteHttpClient(Encoding encoding, ICookieModel cookieModel = null, ICertModel certModel = null)
        {
            //初始化Cookie容器
            Encoding = encoding;
            Timeout = 300000;
        ...
        public static string StringFromResponse(HttpWebResponse response, Encoding encoding)
        {
            ...
            {
                //创建流对象，读取响应流，生成字符串
                var sr = new StreamReader(response.GetResponseStream() ?? throw new InvalidOperationException(), encoding);
